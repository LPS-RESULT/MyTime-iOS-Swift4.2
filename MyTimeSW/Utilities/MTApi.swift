...
class MTApi {
    ...
    static func getUserDetails(fromUsername username: String, result: @escaping (Bool, MTDefaults.UserDetails?) -> Void) {
            ...
            do {
                if (response.response?.statusCode == 200) {
                    ...
                    if let profileRecord = responseJson["records"].array?.first {
                        ...
                        profile.firstName = profileRecord["FirstName"].string
                        profile.lastName = profileRecord["LastName"].string
                        profile.commit();
                    }
                } else {
                    result(false, nil)
